#include <BasicLinearAlgebra.h>

// //Noise estimates
#define ALT_STD 0.5//0.173932 //noise of position observation
#define ACC_STD 0.12

 
#define MODEL_Y_STD 0.5
#define MODEL_V_STD 0.5
#define MODEL_A_STD 0.1

BLA::Matrix<3, 3> I = {1, 0, 0,
                       0, 1, 0,
                       0, 0, 1}; //3x3 identity matrix used for later

BLA::Matrix<3> x = {0, 0, 0}; //state estimate
BLA::Matrix<2> z = {0, 0}; //measurement
BLA::Matrix<3, 3> phi = I; //state transition
BLA::Matrix<2, 3> H = {1, 0, 0,
                        0, 0, 1}; //state to measurement
BLA::Matrix<2, 2> R = {ALT_STD * ALT_STD, 0,
                        0, ACC_STD * ACC_STD}; //measurement covariance
BLA::Matrix<3, 3> Q = {MODEL_Y_STD * MODEL_Y_STD, 0, 0,
                                      0, MODEL_V_STD * MODEL_V_STD, 0,
                                      0, 0, MODEL_A_STD * MODEL_A_STD}; //Process covariance
BLA::Matrix <3, 3> P = I; //Error covariance
BLA::Matrix <3, 2> K; //Kalman gain


void updateKalmanFilter(){
  //Add data
  z(0) = getCalibratedAlt();
  z(1) = getVerticalAcceleration();
  phi = {1, dt, 0.5 * dt * dt,
         0, 1, dt,
         0, 0, 1};

  //Update Kalman Gain
  K = P * ~H * Inverse(H * P * ~H + R);

  //Update Estimate
  x = x + K * (z - H * x);

  //Update Covariance
  P = (I - K * H) * P;

  //Project to next time stamp
  x = phi * x;
  P = phi * P * ~phi + Q;

  //Update log
  logLine[7] = String(x(0));
  logLine[8] = String(x(1));
  logLine[9] = String(x(2));

  printState();
}

void printState(){
  serialTag("Yestimate", x(0));
  serialTag("Vestimate", x(1));
  serialTag("Aestimate", x(2));
  sPrintln();
}

float getYEstimate(){
  return x(0);
}

float getVEstimate(){
  return x(1);
}

float getAEstimate(){
  return x(2);
}