...
double control::getApogee(double y, double v, double deployment, Config& config, double cosPitch){
    double cd = control::getCD(deployment, config) / cosPitch;
...
//Computes +
double control::computeDeployment(double y, double v, Config& config, double cosPitch){
    //Check bounds 
    if(getApogee(y, v, 0, config, cosPitch) < config.TARGET_APOGEE_METERS)
        return rateLimiter.get(0);
    else if(getApogee(y, v, 1, config, cosPitch) > config.TARGET_APOGEE_METERS)
        return rateLimiter.get(1);

    double addition, deployment = 0;
    for(int digits = 1; digits <= 6; digits++){
        addition = 1.0 / (1 << digits); 
        if(getApogee(y, v, deployment + addition, config, cosPitch) > config.TARGET_APOGEE_METERS)
            deployment += addition;
    }
    //Apply rate limit
