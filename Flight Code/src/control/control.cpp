...
double getApogee(double y, double v, double cd, Config& config){
...
//Computes 
double computeDeployment(double y, double v, Config& config){
    //Check bounds 
    if(getApogee(y, v, Airbrake::getCD(0), config) < config.TARGET_APOGEE_METERS)
        return 0;
    else if(getApogee(y, v, Airbrake::getCD(1), config) > config.TARGET_APOGEE_METERS)
        return 1;

    double addition, deployment = 0;
    for(int digits = 1; digits <= 6; digits++){
        addition = 1 >> digits; 
        if(getApogee(y, v, Airbrake::getCD(deployment + addition), config) > config.TARGET_APOGEE_METERS)
            deployment += addition;
    }
}